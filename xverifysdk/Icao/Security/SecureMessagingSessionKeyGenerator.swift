        ...
        if cipherAlgName == "DESede" || cipherAlgName == "3DES" {
            ...
            switch(keyLength) {
                case 112, 128:
                    ...
                    break;
                default:
                    throw NfcEIdReaderError.InvalidDataPassed("Can only use DESede with 128-but key length")
            }
            ...
            switch(keyLength) {
                ...
                default:
                    throw NfcEIdReaderError.InvalidDataPassed("Can only use AES with 128-bit, 192-bit key or 256-bit length")
            }
        } else {
            throw NfcEIdReaderError.InvalidDataPassed( "Unsupported cipher algorithm used" )
        }
